...
async def browse_with_browser(browser_choice, mode, browser_paths):
    """使用指定浏览器进行浏览"""
    if browser_choice == "":
        co = ChromiumOptions()
...
async def main():
    ...
    if args.browser == 'single':
        ...
        await browse_with_browser(choice, args.mode, browser_paths)
    else:
        for browser_name in browser_paths.keys():
            print(f"\n正在使用 {browser_name} 浏览...")
            await browse_with_browser(browser_name, args.mode, browser_paths)
